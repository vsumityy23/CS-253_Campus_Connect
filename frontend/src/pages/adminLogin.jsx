...
export default function AdminLogin() {
  ...
  const login = async (e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || data.msg || "Unauthorized access");
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-6 text-slate-900 font-sans">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-10">
        ...
        <form onSubmit={login} className="space-y-6">
          <div>
            <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Master Password</label>
            <input
              type="password"
              value={password}
